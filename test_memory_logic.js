...
const testMemories = [
  {
    ...
    response: 'To implement OAuth 2.0 with PKCE, you need to: 1. Generate code verifier 2. Create code challenge 3. Redirect to authorization server...',
  ...
  },
  {
    ...
    response: 'PostgreSQL indexing best practices: 1. Index frequently queried columns 2. Use composite indexes for multi-column queries 3. Monitor index usage...',
    tags: ['database', 'postgresql', 'performance', 'indexing']
  },
  {
    context: 'React architecture review',
    query: 'How should I structure reusable React components for our design system?',
    response: 'For reusable React components: 1. Use composition over inheritance 2. Implement proper prop interfaces 3. Follow atomic design principles...',
...
try {
  testMemories.forEach((memory, index) => {
    const addedMemory = memoryManager.addMemory(memory.context, memory.query, memory.response, memory.tags);
...
try {
  ...
  console.log(`✅ Search for "authentication" returned ${authResults.length} results`);
  if (authResults.length > 0) {
    console.log(`   - Top result score: ${authResults[0].score}`);
    console.log(`   - Top result contains: "${authResults[0].entry.query.substring(0, 60)}..."`);
  }
  ...
  console.log(`✅ Search for "nonexistent" returned ${noResults.length} results (expected 0)`);
...
try {
  ...
  if (recentMemories.length > 0) {
    console.log(`   - Most recent: "${recentMemories[0].query.substring(0, 50)}..."`);
    console.log(`   - Timestamp: ${new Date(recentMemories[0].timestamp).toISOString()}`);
  }
...
try {
  ...
  console.log(`   - Oldest timestamp: ${new Date(stats.oldestTimestamp).toISOString()}`);
  console.log(`   - Newest timestamp: ${new Date(stats.newestTimestamp).toISOString()}`);
...
try {
  ...
  if (memories.length > 0) {
    ...
    const requiredFields = ['id', 'timestamp', 'context', 'query', 'response', 'tags'];
